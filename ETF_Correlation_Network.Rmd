...
A popular trend within the realm of funds is the low-cost fund, or index fund. These ETFs track broad idicies or follow broad strategies and are passively managed. Because there is no active management, fees are lower, which allows investors to keep more of their returns.
...
An additional CSV file "ETF_cor_matrix.csv" was created to contain the values for the correlation coefficients that were calculated in a separate R Notebook in the format of a matrix.
...
It was necessary to calulate to correlation coefficients between each two ETFs in order to create the heat map and histogram and to assign the appropriate edge weight to the graph. I first approached that by calculating the correlation coefficients individually for each pair. I then copied the output value from RStudio into the Weight column in the Excel file with the edge list. I also copied the value into another Excel file that was formatted as a matrix of the correlation coefficients. I then uploaded that CSV file into RStudio and converted the data frame into a matrix. While this approach got the job done, it took a very long time of repititious action and copying and pasting. I later learned a much better approach. Instead of calculating the correlation coefficents individually and creating a matrix from scratch, I was able to use the cor() function with the entire ETF_prices data frame as a parameter, and it returned the correlation matrix. This one line of code replaced about two hours of data organization. 

```{r}
#Input CSV file containing edge with weigths 
#Input and print CSV file containing matrix of correlation coefficients between each pair of ETFs
ETF_edges <- read_csv("ETF_edges.csv")
ETF_cor_matrix <- read_csv("ETF_cor_matrix.csv")
ETF_cor_matrix
```

```{r}
#Convert the data frame to a matrix and print the correlation matrix
ETF_correlation_matrix <- as.matrix(ETF_cor_matrix)
ETF_correlation_matrix
```

Calculate correlation matrix
```{r}
ETF_Cor_matrix2 <- cor(ETF_prices[,-1])
ETF_Cor_matrix2
```
...
#Discussion
VXUS, VEA, and VWO emerged as a community both over the entire five year span and over the course of the first year within that. These funds are Vanguard Total International Stock Index Fund ETF (VXUS), Vanguard FTSE Developed Markets Index Fund ETF (VEA), and Vanguard FTSE Emerging Markets Index Fund ETF (VWO). It makes sense that these are highly correlated because they all focus on equities outside of North America while the other seven ETFs focus on American equities. One possible conclusion is that there is not much of a difference between the three internation ETFs despite one being focused on developed economies with another being focused on emerging economies. The correlation coefficient for those two ETFs over all five years was 0.92, indicating a very strong, positve correlation. Additionally, the fact that the other seven ETFs formed a community, combined with the fact that the majority of correlation coefficients were greater than 0.9, indicates that the overall performance of these funds was quite simple despite the funds varying in their approaches, whether that be growth, value, high-yield, dividend growth, large-cap, mid-cap, or small-cap. This high correlation implies that investment performance would not differ much between these seven funds, which means that attempting to diversify by investing in several of these funds would not be effective. It's important to note, however, the the five year period being analyzed, is entirely encompassed by a bull market, so it's difficult to say if these conclusions would hold true in a bear market, which is perhaps more important because a bear market is when reducing risk becomes most prudent.
